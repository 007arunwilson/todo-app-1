...
function todoListPage() {
    ...
    document.getElementById('user').innerText=JSON.parse(localStorage.getItem('user')).name;
...
smallDate.textContent=days[date.getDay()]+" "+date.getDate()+" "+months[date.getMonth()];
...
taskNameinp.setAttribute('placeholder','e.g. Read every day p3 @goals #Learning');
...
if(date.getMonth()<9) {
    taskDate.setAttribute('min',date.getFullYear()+'-'+'0'+parseInt(date.getMonth()+1)+'-'+date.getDate());
}
else {
    taskDate.setAttribute('min',date.getFullYear()+'-'+parseInt(date.getMonth()+1)+'-'+date.getDate());
}
...
function allTasks(){
    ...
    ajaxObj.setURL(`https://to-do-list-b734d.firebaseio.com/todos.json?orderBy="firebaseid"&equalTo="${userKey}"`);
    ajaxObj.setCallback((response) => {
...
function listAllTasks(responseObj) {
    var taskName="";
    var dateArray=[],keyArray=[],i=0;
    for(var task in responseObj) {
        dateArray[i]=new Date(responseObj[task].date);
        i++;
    }
    dateArray.sort(function(a, b){return b.getTime() - a.getTime()});
    ...
    for(var i=0;i<editTask.length;i++) {
        editTask[i].addEventListener('click',function(event){
            ...
            taskNameinp.addEventListener('keyup',function(event){
                ...
                ajaxObj.setURL('https://to-do-list-b734d.firebaseio.com/todos/'+event.target.id.split('n')[1]+'.json');
...
function login() {
    ...
    if(validateLogin(formElements)) {
        ...
        ajaxObj.setLoadingFn(function() {
            document.getElementById('bodyWrapper').innerHTML='<div id="loading"><img src="../assets/images/loading.gif" alt="loading..." width=50></div>';
        });
